<script lang="ts">
type Person = { id: string; name: string; tags: string[]; _count?: number };
...
const filteredPersons = $derived.by((): Person[] => {
	const needle = normalize(q);
	if (!needle) return data.persons;
	return data.persons.filter((person: Person) => normalize(person.name).startsWith(needle));
});
</script>
...
<section class="surface p-6 sm:p-8">
	<header class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
		<div class="space-y-3">
			...
			<div>
				<h1 class="text-2xl font-semibold tracking-tight">Mis Perfiles</h1>
				<p class="text-sm text-zinc-600 dark:text-[hsl(var(--muted))]">
					Creá perfiles para organizar tus lecturas por persona o tema.
				</p>
			</div>
		</div>
			...
			{#if data.persons.length > 0}
				<form class="w-full max-w-xs md:w-56">
					...
					<input
						id="q"
						name="q"
						class="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[hsla(var(--accent)/0.3)] dark:border-[hsl(var(--border))] dark:bg-[hsl(var(--surface))] dark:text-[hsl(var(--text))]"
	...
	{#if filteredPersons.length === 0}
		<div class="mt-8 rounded-2xl border border-zinc-200 bg-zinc-50 p-5 dark:border-[hsl(var(--border))] dark:bg-[hsl(var(--surface-alt))]">
			...
			<div class="mt-1 text-sm text-zinc-600 dark:text-[hsl(var(--muted))]">
				Creá un perfil para separar lecturas (ej: "Yo", "Pareja", "Trabajo").
			</div>
			<a
	...
	{:else}
		<ul class="mt-6 space-y-3">
			{#each filteredPersons as person}
				<li>
					<a class="surface person-card" href={`/app/profiles/${person.id}`}>
						...
						<div class="min-w-0 flex-1 space-y-2">
							<div class="flex items-center justify-between gap-3">
								...
								<div class="text-xs font-semibold text-zinc-500 dark:text-[hsl(var(--muted))]">
									{person._count ?? 0} {person._count === 1 ? 'lectura' : 'lecturas'}
								</div>
							</div>
