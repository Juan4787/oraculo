-- Fix create_reading param names to match RPC while keeping unique cards per reading
...
begin
	...
	elsif owner_type = 'person' then
		select count(*) into reading_count
		from public.readings r
		where r.workspace_id = workspace_id
			and r.owner_type = 'person'
			and r.owner_person_id = owner_person_id;
	...
	with eligible as (
		...
		where c.workspace_id = workspace_id
			...
			and (
				selected_deck_ids is null
				or cardinality(selected_deck_ids) = 0
				or c.deck_id = any(selected_deck_ids)
			)
	),
	ordered as (
		select
	...
	values (
		workspace_id,
		owner_type,
		case when owner_type = 'user' then caller_id else null end,
		case when owner_type = 'person' then owner_person_id else null end,
		caller_id,
		spread_id,
		selected_deck_ids,
		seed
	...
	for i in 1..card_n loop
		...
		into back_image_path, back_phrase
		from public.card_backs cb
		where cb.workspace_id = workspace_id
