-- Enforce message-only backs and exclude cards without valid backs
delete from public.card_backs cb
using public.cards c
where cb.card_id = c.id
  and (
    cb.back_image_path ilike '/cards/arcangel-%'
    or (c.image_path is not null and cb.back_image_path = c.image_path)
  );
...
begin
	...
	with eligible as (
		...
		from public.cards c
		join public.card_backs cb on cb.card_id = c.id
		where c.workspace_id = create_reading.workspace_id
			and c.status = 'published'
			and cb.status = 'published'
			and cb.back_image_path is not null
			and cb.back_image_path not ilike '/cards/arcangel-%'
			and (c.image_path is null or cb.back_image_path <> c.image_path)
			and (
	...
	if selected_card_ids is null or array_length(selected_card_ids, 1) < card_n then
		raise exception 'No hay suficientes cartas publicadas con reversos válidos para esta tirada';
	end if;
